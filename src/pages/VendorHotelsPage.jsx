...
const VendorHotelsPage = () => {
  ...
  const handleEditHotel = async (hotelId) => {
    try {
      ...
      setImagePreview(hotelData.image_url || "");
      setImageFile(null);
      setSelectedHotel(hotelId);
      setModalMode("edit");
      setShowModal(true);
  ...
  const handleRemoveImage = () => {
    setImageFile(null);
    setImagePreview("");
    setHotelForm((prev) => ({ ...prev, image_url: "" }));
  };
  ...
  const handleSubmitHotel = async (e) => {
    ...
    try {
      ...
      formData.append("street", hotelForm.street.trim());
      formData.append("city", hotelForm.city.trim());
      formData.append("state", hotelForm.state.trim());
      formData.append("pin", hotelForm.pin.trim());
      formData.append("primary_email", hotelForm.primary_email.trim());
      formData.append("primary_phone", hotelForm.primary_phone.trim());
      ...
      // For edit mode, include existing image URL if no new image
      if (modalMode === "edit" && !imageFile && hotelForm.image_url) {
        formData.append("existing_image_url", hotelForm.image_url);
      }
      ...
      console.error("Error saving hotel:", error);
      toast.error(error.response?.data || "Failed to save hotel");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleViewDetails = async (hotelId) => {
    ...
    try {
      ...
      const [hotelRes, emailsRes, phonesRes] = await Promise.all([
        ...
        axios.get(`/hotels/${hotelId}/emails/`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get(`/hotels/${hotelId}/phones/`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
      console.log(emailsRes.data.data);
            ...
            <form onSubmit={handleSubmitHotel}>
              <div className="modal-body">
                ...
                <div className="form-group">
                  ...
                  >
                    Upload an image (JPG, PNG, etc. - Max 5MB)
                  </small>
          ...
          >
            ...
            <div className="modal-body">
              ...
              <div className="details-section">
                ...
                <div className="details-grid">
                  <div className="detail-item">
                    <strong>Location:</strong> {detailsData.street},{" "}
                    {detailsData.city}, {detailsData.state} - {detailsData.pin}
                  </div>
                  <div className="detail-item">
                    <strong>Total Rooms:</strong> {detailsData.total_rooms}
                  </div>
                ...
                <div className="multi-value-section">
                  ...
                  <div className="item-list">
                    ...
                    {detailsData.emails && detailsData.emails.length > 0
                      ? detailsData.emails
                          .filter(
                            (email) => email !== detailsData.primary_email
                          )
                          .map((email, idx) => (
                            <div key={idx} className="item-badge">
                              {email}
                ...
                <div className="multi-value-section">
                  ...
                  <div className="item-list">
                    ...
                    {detailsData.phones && detailsData.phones.length > 0
                      ? detailsData.phones
                          .filter(
                            (phone) => phone !== detailsData.primary_phone
                          )
                          .map((phone, idx) => (
                            <div key={idx} className="item-badge">
                              {phone}
