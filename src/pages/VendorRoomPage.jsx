  ...
  const handleSubmitRoom = async (e) => {
    ...
    try {
      ...
      if (modalMode === "create") {
        ...
        const response = await axios.post(`/hotels/${hotelId}/rooms`, roomsPayload, {
          headers: {
        ...
        const response = await axios.put(`/hotels/${hotelId}/room/${selectedRoom}`, payload, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          },
    ...
    <div className="vendor-management">
      <div className="dashboard-header">
        <div className="header-content">
          ...
          {hotelInfo && (
            <p>
              Manage rooms for {hotelInfo.name} ({hotelInfo.city}, {hotelInfo.state})
            </p>
          )}
        </div>
      </div>
                      ...
                      <tr key={room.room_id}>
                        ...
                        <td>
                          <div className="action-buttons">
                            <button className="btn-action btn-edit" onClick={() => handleEditRoom(room)} title="Edit">
                              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>{modalMode === "create" ? "Add New Room(s)" : "Edit Room"}</h2>
                    ...
                    {multipleRooms.map((room, index) => (
                      <div key={index} className="room-form-section">
                        ...
                        <div className="form-row">
                          ...
                          <div className="form-group">
                            <label htmlFor={`bed_type-${index}`}>Bed Type *</label>
                            <input
                              type="text"
                              id={`bed_type-${index}`}
                              name="bed_type"
                        ...
                        <div className="form-row">
                          <div className="form-group">
                            <label htmlFor={`cost_per_night-${index}`}>Cost Per Night *</label>
                            <input
                              type="number"
                          ...
                          <div className="form-group">
                            <label htmlFor={`max_capacity-${index}`}>Max Capacity *</label>
                            <input
                              type="number"
                        ...
                        {index < multipleRooms.length - 1 && <hr className="form-divider" />}
                      </div>
                    ))}

                    <button type="button" className="btn-secondary" onClick={handleAddRoomForm}>
                      + Add Another Room
                    </button>
                  </>
                ) : (
                  <>
                    <div className="form-row">
                      <div className="form-group">
                        ...
                        <input type="text" id="room_id" name="room_id" value={roomForm.room_id} disabled />
                      </div>
                      <div className="form-group">
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn-primary" disabled={loading}>
                  {loading
                    ? "Saving..."
                    : modalMode === "create"
                      ? `Create ${multipleRooms.length} Room(s)`
                      : "Update Room"}
                </button>
              </div>
            </form>
