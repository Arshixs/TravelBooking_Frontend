...
// import axios from "../utils/axios"; // Keep commented for mock data flow
...
const PackageFormModal = ({ packageData, onClose, onSave }) => {
    ...
    useEffect(() => {
        if (packageData) {
            // When editing, populate the form with the package data from props
            setFormData({
                ...packageData,
    ...
    const handleSubmit = async (e) => {
        ...
        // This is the mock data version that passes data to the parent component
        onSave(formData);
        alert("Package saved locally!");

        /* // This is the backend version you can uncomment later
        try {
            if (packageData) {
            ...
            onSave(); // onSave in the backend version just refreshes the list
        } catch (error) {
